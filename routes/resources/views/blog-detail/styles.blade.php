...
<style>
                ...
                foreach ($latestblog as $single) {
                    ...
                    if ($blogSettings->use_generic) {
                      ...
                      // dd($blog_desc->blog_desc_color);
                        ...
                        $title_color = 'color:' . ($generic_blog_title->color ? $generic_blog_title->color . ';' : '#000;');
                      ...
                      ?>
                       .blog_readmore_btn_<?=$single->id?>{
                          background:<?=$blogSettings->use_generic ? $blogSettings->read_more_button_bg_color : $single->read_more_button_bg_color ?>;
                        ...
                        .otherblog-desc-<?=$single->id?>{
                            <?= $desc_font_family ?><?= $desc_color ?>
                        }
                        .otherblog-desc-<?=$single->id?>{
                            <?= $desc_font_family ?><?= $desc_color ?>
                        }
                        .otherblog-title-<?=$single->id?>{
                            <?= $title_font_family ?><?= $title_color ?>
                        }
                        .otherblog-date-<?=$single->id?>{
                            <?=$date_font_family?><?=$date_color?>
                        }
                        .otherblog-cate-<?=$single->id?>{
 ...
 <?php 
          if ($blogSettings->use_generic) {
            ...
            $title_font_family = 'font-family:' . ($generic_blog_title->fontfamily ? getfontfamily($generic_blog_title->fontfamily) . ' !important;' : ';');
            $title_color = 'color:' . ($generic_blog_title->color ? $generic_blog_title->color . '!important;' : '#000 !important;');
            ...
            $desc_font_family = 'font-family:' . ($blog->desc_font ? getfontfamily($blog->desc_font) . ';' : ';');
            $desc_color = 'color:' . ($blog->desc_color ? $blog->desc_color . ';' : '#000;');
            

            $title_font_family = 'font-family:' . ($blog->title_font ? getfontfamily($blog->title_font) . ';' : ';');
            $title_color = 'color:' . ($blog->title_color ? $blog->title_color . ';' : '#000;');

            $cate_font_family = 'font-family:' . ($blog->category_font ? getfontfamily($blog->category_font) . ';' : ';');
            $cate_color = 'color:' . ($blog->category_color ? $blog->category_color . ';' : '#000;');
            
            $date_font_family = 'font-family:' . ($blog->date_font ? getfontfamily($blog->date_font) . ';' : ';');
            $date_color = 'color:' . ($blog->date_color ? $blog->date_color . ';' : '#000;');
          }
          ?>
            .blog-desc-<?=$blog->id?>{
                <?= $desc_font_family ?><?= $desc_color ?>
            }
            .blog-desc-icon-<?=$blog->id?>{
             <?= $desc_color ?>
            }
            .blog-title-<?=$blog->id?>{
                <?= $title_font_family ?><?= $title_color ?>
