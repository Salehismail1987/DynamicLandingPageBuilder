...
@section('content')
  <div class="container-fluid" id="container-wrapper">
      ...
      <form class="data-form" role="form" method="post" enctype="multipart/form-data" action="{{ url('createNewEmailTemplate') }}">
        @csrf
        <div class="card-body">
            <div class="row">
                <div class="col-md-3">
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-group action_fields action_address" style="display:none;">
                                  ...
                                  foreach($addresses as $address){
                                    ?>
                                      <option value="<?=$address->id?>"><?=$address->address_title?></div>
                                    <?php 
                                  }
                                  ?>
                                  </select>
...
$(document).on('change', '.selecttemplate', function() {
    ...
    $.ajax({
      ...
      type: "POST",
      dataType: 'JSON',
      data: {temp_id: value, _token: "{{ csrf_token() }}"},
...
$(document).on('change', '#override_generic_settings', function() {
  ...
  function addAnotherImage(){
    var newupload = ' <div class="col-md-12" style="margin-top:10px;border-top:2px solid lightgrey;padding-top:10px" id="img-'+total+'" ><button type="button" class="btn btn-info" style="float:right" onclick="removeImageDiv('+total+')">Remove</button> <div class="uploadImageDiv"><button type="button" class="btn btn-primary btnuploadimagenew" data-toggle="modal" data-target="#modalImagesforUploads">Upload Another Image</button><input type="hidden" name="imagefromgallery" class="imagefromgallery"> <input class="dataimage" type="hidden" name="userfile[]"> <div class="col-md-6 imgdiv" style="display:none"> <br> <img src="" width="100%" class="imagefromgallerysrc"> <button type="button" class="btn btn-primary btnimgdel btnimgremove" >X</button></div></div> </div>';
