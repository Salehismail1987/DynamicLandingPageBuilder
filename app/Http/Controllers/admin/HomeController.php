...
class HomeController extends Controller
{
    ...
    public function home(Request $request){
        ...
        $data['formSettings'] =  formsSettings::first();
        ...
        $data['rotatingSchedule'] = rotatingSchedule::all();
        ...
        $data['customFormsAll'] = customForms::orderBy('title','ASC')->get();
	    ...
	    if($_POST){
			...
			try{
                ...
                if($email !=""){
                    ...
                    sendmail($email,'New mail','string_template',$data,$files_array);
                }
			}catch(Exception $exp){
	    ...
	    if($_POST){
			...
			if($form_id=='7'){
				...
				if($form_id=='8'){
					...
					$form = customUserForm::create(array('in_folder'=>'0','form_id'=>$form_id,'date_time'=>date('Y-m-d H:i:s'),'fields_data'=>json_encode($formdata)));
					$msg_id = $form->id;
					$message = "<br> Click Link below to view Response<br>".url('edituserform/'.$msg_id);
				}
			}
