...
if($blog_title->enable=='1' || ($blogs && count($blogs)>0) || isset($_GET['editwebsite'])){ ?>
    ...
    <div class="blog-bg" id="blogsection" class="p-3 pt-4" style="">
    ...
    <?php if(isset($page) && $page=='list'){ ?>
      ...
      <?php }else{ ?>
        ...
        <?php } ?>
      <?php } ?>
      <div class="position-relative blog_outline" >
      @if(isset($_GET['editwebsite']))
          <div class="">
                  <div class="d-flex align-items-center">
                      <x-tutorial-action-buttons  title='Blog' :buttons="isset($tutorial_action_buttons['blog']) ? $tutorial_action_buttons['blog']:'' " url='blog?block=blog_list' :status="$setting->section_enabled"/>
                  </div>
          </div>
      @endif
        <?php if(isset($page) && $page=='list'){ ?>
