            ...
            ?>
                <?php foreach ($formsLinks as $single) { ?>
                    @if(nl2br($single->link_text))
                        <a  class="LSformslinks" href="#" data-toggle="modal" data-id='{{getCustomformEncodedID($single->link_forms)}}' data-is_custom_modal="YES" data-target="#modalcustomforms<?=getCustomformEncodedID($single->link_forms)?>"><?= nl2br($single->link_text) ?></a>
                    @endif
                <?php } ?>
              <?php } ?>
            </div>
