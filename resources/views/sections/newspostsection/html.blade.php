                  ...
                  <div style="text-align: left;">
                    <?php if ($single->action_button_active == '1') {
                      ...
                      if ($single->action_button_link == 'link') {
                        ...
                        if (!preg_match('/^(http:\/\/|https:\/\/|www\.)/i', $input_link)) {
                    ...
                    elseif ($single->action_button_link == 'customforms') {
                      ...
                      elseif ($single->action_button_link == 'eventForms') {
                        ...
                        $popupform = 'data-toggle="modal" data-is_custom_modal="YES" data-target="#modalcustomforms' . getCustomformEncodedID($single->event_form_id) . '"';
                      } 
                      ...
                      <a href="<?= $input_link ?>" onclick="playPauseAudio('newspostAudio_<?= $single->id ?>')" style="<?= 'color:' . ($single->action_button_discription_color ? $single->action_button_discription_color . '!important;' : '#000;'); ?>" class="btn btn-default {{$mt_1rem}} btn-adjustable mb-1 text-bold new-action-btn post-action-{{$single->id}} " style=""> 
                      ...
                      <br>
                      @else
                      <a href="<?= $input_link ?>" id="<?= $single->id . 'newspost' ?>" style="<?= 'color:' . ($single->action_button_discription_color ? $single->action_button_discription_color . '!important;' : '#000;'); ?>" <?php if ($single->action_button_link == "image_popup") 
                      { ?> data-toggle="<?= $data_toggle ?>" data-target="<?= $data_target ?>" onclick="openSlider(<?= htmlspecialchars($single->popup_images) ?>, '<?= url('assets/uploads/' . get_current_url()) ?>');" <?php 
                      } ?> <?php if ($single->action_button_link == 'text_popup') { ?> onclick="openPopupText('actNewsButtonText<?= $single->id ?>')" <?php } ?> 
                      <?php if ($single->action_button_link == "video") 
                      { ?> data-toggle="<?= $data_toggle ?>" data-target="<?= $data_target ?>" onclick="openVideo('<?= $single->id . 'newspost' ?>')" <?php } ?> 
                      <?= isset($single->action_button_action_audio) ? 'onclick=playPauseAudio("newsPostAudio")' : '' ?>
                       target="<?= $target ?>" <?= $popupform ?> class="btn btn-default {{$class}} {{$mt_1rem}} mb-1 text-bold new-action-btn post-action-{{$single->id}} " style=""><?= $single->action_button_discription ?></a>
