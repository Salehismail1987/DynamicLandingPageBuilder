                    ...
                    ?>
                       @if($single->action_button_link == "audioiconfeature" && isset($single->action_button_audio_icon_feature)) 
                          <span class="video_desc_icon_<?=$single->id?>" onclick="playPauseAudio('gallvidAudio_<?=$single->id ?>')" >
                              <a href="<?= $input_link ?>" 
                              style="" class="btn btn-default mt-1 mb-1 btn-adjustable text-bold new-action-btn video-action-{{$single->id}} {{$audioclass}}" style=""><span class="text"><span style="position:absolute;left:10px;">
                              <i class="fa fa-volume-up" aria-hidden="true"></i></span><?= $single->action_button_discription ?></span></a>
