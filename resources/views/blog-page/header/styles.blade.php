...
body {
    ...
    width: 100%;
    position: relative;
    @if($siteSettings->site_background_color && isset($siteSettings->site_background_color) && $siteSettings->site_background_color != 'null') 
...
.titlefontfamily {
    font-weight: bold;
    line-height: 1;
    <?php if ($master_title->fontfamily) { ?>font-family: <?=  getfontfamily($master_title->fontfamily) ?>; <?php } ?>
}
...
.dailyhoursdae {
    ...
    border: 1px solid<?= $daily_hours_day_block->color ?> <?php } ?>
...
.dailyhourslist .singledailyhour {
    padding-bottom: 10px;
    <?php if ($rotatingScheduleSettings->background) { ?>background: <?= $rotatingScheduleSettings->background ?> !important;
    <?php } ?>
}
...
.LSlinksLink {
    ...
    <?php } ?><?php if ($hyper_link_link->fontfamily) { ?>font-family: <?= getfontfamily($hyper_link_text->fontfamily) ?> !important;
    <?php } ?>
}
...
.sicialmediaicon {
    ...
    <?php if ($social_media_icon->color) { ?>color: <?= $social_media_icon->color ?> !important;
...
#header{
    padding:5px;<?php if ($alert_banner_text->bg_color) {
