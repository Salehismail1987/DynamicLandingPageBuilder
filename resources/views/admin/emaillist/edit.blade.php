                      ...
                      <?php if ($contact_database->fields) { ?>
                        ...
                        <?php } elseif ($sngl->fieldtype == 'radio') { ?>
                            ...
                            <?php $checked='checked';foreach($sngl->options as $singleop){ ?>
                                ...
                                value="<?=$singleop?>" <?=isset($userdata[strtolower(str_replace(' ', '_', $sngl->fieldname))]) && strtolower($userdata[strtolower(str_replace(' ', '_', $sngl->fieldname))])==strtolower($singleop)?'checked':''?>/> <?=$singleop?><br>
                            <?php } ?>
                            <div class="validate"></div>
                            </div>
                        ...
                        <?php }elseif ($sngl->fieldtype == 'multiselect') { ?>
                            <div class="form-group">
                                <label><?=$sngl->fieldname?></label><br>
                                <select name="<?= strtolower(str_replace(' ', '_', $sngl->fieldname)) ?>[]" class="myinput2 inputchange height-max-content pt-15px" multiple>
                                    <?php $checked='checked';foreach($sngl->options as $singleop){
                                        $multidata = isset($userdata[strtolower(str_replace(' ', '_', $sngl->fieldname))]) ? $userdata[strtolower(str_replace(' ', '_', $sngl->fieldname))] : '' ;
                                        if(!is_array($multidata) && $multidata !=''){

                                            if(strpos($multidata,'/')){
                                                $multidata= trim(strtolower($multidata));

                                               $multidata= explode('/', $multidata);
                                            }

                                        }?>
                                        <option value="<?=strtolower($singleop)?>" <?=(isset($multidata) && is_array($multidata) && in_array(strtolower($singleop),$multidata))?'selected':''?>> <?=$singleop?> </option>
                                    <?php } ?>
                                </select>
                            </div>
                        <?php }elseif ($sngl->fieldtype == 'select') { ?>
                            <div class="form-group">
                                ...
                                <select name="<?= strtolower(str_replace(' ', '_', $sngl->fieldname)) ?>" class="myinput2 inputchange">
                                    <?php $checked='checked';foreach($sngl->options as $singleop){ ?>
                                        <option value="<?=strtolower($singleop)?>" <?=isset($userdata[strtolower(str_replace(' ', '_', $sngl->fieldname))]) && strtolower($userdata[strtolower(str_replace(' ', '_', $sngl->fieldname))])==strtolower($singleop)?'selected':''?>> <?=$singleop?></option>
                                    <?php } ?>
                                </select>
                            </div>
                        ...
                        <?php }  elseif ($sngl->fieldtype == 'file')  { ?>
                            ...
                            <?php
                                $image = isset($userdata[strtolower(str_replace(' ', '_', $sngl->fieldname))])?$userdata[strtolower(str_replace(' ', '_', $sngl->fieldname))]:'';
                                if(isset($image) && $image !='' && $image !='file url')
                                {
                                    ?>
                                       <img src="<?=url('assets/uploads/').'/'.get_current_url().'/'.$image?>" width="100%">
                                    <?php
                                }
                            ?>
                            <?php
                             if(isset($userdata['files'][strtolower(str_replace(' ', '_', $sngl->fieldname))])) {
                                 ?>
