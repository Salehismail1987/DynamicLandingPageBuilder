  ...
  <div class="row">
    ...
    <div class="col-md-6">
        ...
        <div class="card mb-4 card-border">
            <div class="card-body">
            <form action="<?= base_url('updateeventresponse?block='.((isset($block) && $block)? $block :'')) ?>" method="post" enctype="multipart/form-data" role="form" class="php-email-form no-box-shadow p-0">
                @csrf
                    ...
                    <?php } elseif ($sngl->fieldtype == '5_star_min' || $sngl->fieldtype == '4_star_min') { ?>
                        <div class="col form-group mb-10">
                            <div class="form-group mb-10">
                                ...
                                <input type="radio" name="rating" value="5" <?= ($sngl->required) ? 'required' : '' ?> <?= isset($feildsdata['rating']) && $feildsdata['rating'] == 5? 'checked':''?>>
                                <label class="text-black ">5-Star Rating</label>
                                <br>
                                <input type="radio" name="rating" value="4" <?= isset($feildsdata['rating']) && $feildsdata['rating'] == 4? 'checked':''?>>
                                <label class="text-black">4-Star Rating</label>
                                <br>
                                <input type="radio" name="rating" value="3" <?= isset($feildsdata['rating']) && $feildsdata['rating'] == 3? 'checked':''?>>
                                <label class="text-black">3-Star Rating</label>
                                <br>
                                <input type="radio" name="rating" value="2" <?= isset($feildsdata['rating']) && $feildsdata['rating'] == 2? 'checked':''?>>
                                <label class="text-black">2-Star Rating</label>
                                <br>
                                <input type="radio" name="rating" value="1" <?= isset($feildsdata['rating']) && $feildsdata['rating'] == 1? 'checked':''?>>
                                <label class="text-black">1-Star Rating</label>
                                <div class="validate"></div>
                            </div>
                            <div class="form-group">
                                <label><b><?=$sngl->fieldname?> <?= ($sngl->required) ? '<span class="text-red">*</span>' : '' ?></b></label><br>
                                <textarea class="myinput2" name="<?= strtolower(str_replace(' ', '_', $sngl->fieldname)) ?>" rows="5" data-rule="<?= ($sngl->required) ? 'required' : '' ?>" placeholder="<?= $sngl->fieldname ?>" <?= ($sngl->required) ? 'required' : '' ?>><?=isset($feildsdata[strtolower($sngl->fieldname)])?$feildsdata[strtolower($sngl->fieldname)]:''?></textarea>
                                <div class="validate"></div>
                            </div>
                        </div>
                    <?php } ?>
