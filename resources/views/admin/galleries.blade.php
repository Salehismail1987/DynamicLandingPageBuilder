                    ...
                    <?php if (count($galleryVideo) > 0) { ?>
                      ...
                      foreach ($galleryVideo as $row) {
                        ...
                        <tr class="galleryvideossections" data-sectionid="<?= $row->id ?>" onclick="showGalleryVideoActionModal(<?= $row->id ?>)">
                          ...
                          <?php
$description = preg_replace('/color: rgb\([^)]*\);/', 'color: #212529;', $row->desc); // Replace color styles
$description = preg_replace(['/<img\b[^>]*>/i', '/<video\b[^>]*>.*?<\/video>/is'], ['', ''], $description); // Remove <img> and <video> tags
?>
                          <td class="hide-mobile">
                            <div class="limit-text">
                              <?= substr($description, 0, 300) ?> </div>
                          </td>
