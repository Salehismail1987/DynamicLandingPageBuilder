...
<div id="content">
            ...
            <?php foreach ($one_step_images as $step) {
              if ($step->category == $category->category) {  ?>
                ...
                <div class="d-lg-flex d-md-block justify-content-between flex-wrap step-container-<?= $step->id ?>" data-id="<?= $step->id ?>" data-name="<?= $step->name ?>">
                  ...
                  <div class="col">
                    <p data-duration="<?= $step ?>" class="nopadd">Timed <?= $step->first_duration > 60 ? $step->first_duration / 20 . " Hours" : $step->first_duration . " Minutes" ?></p>
                    <?php if ($step->status == '1') { ?>
                      <p data-duration='<?= htmlspecialchars(json_encode([
                                          'id' => $step->id,
                                          'category' => $step->category,
                                          'name' => $step->name,
                                          'first_image_id' => $step->first_image_id,
                                          'first_image_location' => $step->first_image_location,
                                          'second_image_id' => $step->second_image_id,
                                          'second_image_location' => $step->second_image_location,
                                          'first_duration' => $step->first_duration,
                                          'second_duration' => $step->second_duration,
                                          'status' => $step->status,
                                          'notification_status' => $step->notification_status,
                                          'active_time' => $step->active_time,
                                          'start_time' => (new DateTime($step->start_time))
                                            ->modify("+{$step->active_time} minutes")
                                            ->format(DateTime::ATOM),  // Format as ISO 8601
                                          'conditions' => $step->conditions,
                                          'conditions_color' => $step->conditions_color,
                                          'default_button_color' => $step->default_button_color,
                                          'default_button_text_color' => $step->default_button_text_color,
                                          'text_enabled' => $step->text_enabled,
                                          'first_image_text_id' => $step->first_image_text_id,
                                          'second_image_text_id' => $step->second_image_text_id,
                                          'created_at' => $step->created_at,
                                          'updated_at' => $step->updated_at
                                        ]), ENT_QUOTES, 'UTF-8') ?>' class="show-counter">
                        <?php
                  ...
                  <div class="col">
                    <p data-duration="<?= $step->second_duration ?>" class="nopadd">Timed <?= $step->second_duration > 60 ? $step->second_duration / 20 . " Hours" : $step->second_duration . " Minutes" ?></p>
  ...
  <script>
    ...
    function updateDuration() {
      ...
      $('.show-counter').each(function() {
        const $this = $(this);
        const data = $this.data('duration'); // Assuming this is JSON parsed

        if (!data || !data.start_time || !data.active_time) {
          console.log('Invalid data:', data);
          $this.text('Invalid Data');
          return;
        }

        // Convert start_time from ISO string to Date object
        const startTime = new Date(data.start_time);

        // Add active time to start time
        const activeTime = parseInt(data.active_time, 10);
        startTime.setMinutes(startTime.getMinutes());

        // Get the current time and adjust for the timezone difference
        const currentTime = new Date();
        currentTime.setHours(currentTime.getHours() - 7); // Adjusting by -7 hours

        // Calculate time difference in milliseconds
        const timeDiff = startTime - currentTime;

        // Convert timeDiff to hours and minutes
        const hoursDiff = Math.floor(timeDiff / 3600000); // convert to hours
        const minutesDiff = Math.floor((timeDiff % 3600000) / 60000); // convert to minutes

        let output = '';
        if (timeDiff > 0) {
          if (hoursDiff > 0) {
            output += `${hoursDiff} Hours `;
          }
          if (minutesDiff > 0) {
            output += `${minutesDiff} Minutes Remaining`;
          }
        } else {
          output = "Time expired";
        }
