    ...
    <div class="content-body">
        <div class="wrapper">
                    ...
                    <ul class="list-unstyled components">
                        ...
                        <li class="@if($controller == 'CRM' || $controller == 'forms'){{'active'}}@endif">
                            ...
                            <ul class="collapse list-unstyled submenu @if($controller == 'CRM' || $controller == 'forms'){{'show'}}@endif" id="toolsSubmenu">
                                ...
                                <li class="@if($controller == 'Attendhub'){{'active'}}@endif">
                                    <a href="{{ url('attendhub') }}">
                                        <svg width="22" height="23" viewBox="0 0 22 23" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M21.7744 2.21736V20.7826C21.7744 22.0299 20.8838 23 19.7494 23H2.79941C1.6791 22.9949 0.774414 22.0402 0.774414 20.7775V2.21736C0.774414 0.980361 1.65566 0 2.79941 0H19.7541C20.8791 0 21.7744 0.964963 21.7744 2.21736ZM20.026 20.7775V2.21736C20.026 2.06338 19.9041 1.91966 19.7541 1.91966H19.3182L14.1479 5.74872L11.2744 3.18746L8.40566 5.74872L3.23535 1.91453H2.79941C2.64941 1.91453 2.52754 2.05825 2.52754 2.21223V20.7775C2.52754 20.9315 2.64941 21.0752 2.79941 21.0752H19.7541C19.9041 21.0803 20.026 20.9366 20.026 20.7775ZM7.81504 7.90449V9.80362H4.36973V7.90449H7.81504ZM7.81504 11.7233V13.6378H4.36973V11.7233H7.81504ZM8.33535 4.16269L10.8666 1.91966H5.31191L8.33535 4.16269ZM18.1791 7.90449V9.80362H8.9916V7.90449H18.1791ZM18.1791 11.7233V13.6378H8.9916V11.7233H18.1791ZM14.2135 4.16269L17.2369 1.91966H11.6869L14.2135 4.16269ZM18.1791 15.5523V17.4669H13.5197V15.5523H18.1791Z" fill="#ADADAD" />
                                        </svg>
                                        <span>Attenhub</span>
            ...
            @if(session()->has('success'))
                <script>
                    // Trigger the alert using cuteAlert
                    cuteAlert({
                        type: "success",
                        title: "",
                    ...
                    // Disable auto-close for the '/attendhub' path
                    const currentPath = window.location.pathname;
            
                    if (currentPath === "/attendhub") {
                    }
                    else if(window.location.pathname.includes('editeventform')) {
                        setTimeout(() => {
                            const alertWrapper = document.querySelector(".alert-wrapper");
                            if (alertWrapper) {
                                alertWrapper.remove(); // Remove the alert after the timeout
                            }
                        }, 4500); // 1500ms timeout for other pages
                    }
                    else {
                        setTimeout(() => {
                            const alertWrapper = document.querySelector(".alert-wrapper");
                        ...
                        }, 1500); // 1500ms timeout for other pages
                    }
                </script>
            @endif
            @if($errors->any())
                    ...
                    <div class="container">
                        ...
                        <div class="image_resolutions landscape">
                            <label class=" w-100 d-flex">
                                <input type="radio" class="image_resolution" name="image_resolution" value="0">
...
<div class="modal fade" id="modalImagesGallery" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true" data-keyboard="false" data-backdrop="static">
    <div class="modal-dialog modal-dialog-centered upload-image-modal modal-dialog-scrollable" role="document" style="z-index: 999;">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body">
                ...
                <div class="row justify-content-center">
                    <div class="col-md-10 d-flex justify-content-around mb-2">
                        ...
                        <button type="button" style="width: 47%;" class="btn btn-primary no-radius open-btns-modal modal-action-buttons">{{$category->name}}</button>
                        @endif
                        @endforeach
                    </div>
                </div>
                    ...
                    <div class="col-md-10 d-flex justify-content-around mb-2">
                        ...
                        <?php if ($single->active == '0') {
                            ...
                            <span onclick="playPauseAudio('newspostAudio_<?= $single->id ?>')" style="<?= 'color:' . ($single->post_desc_color ? $single->post_desc_color . ';' : '#000;'); ?>">
                                ...
                                <a href="<?= $input_link ?>"
                                    style="width:47%;" class="btn btn-default mb-1 text-bold new-action-btn post-action-{{$single->id}} " style=""><?= $single->action_button_discription ?></a>
                            ...
                            @else
                            <a style="width:47%;" href="<?= $input_link ?>" class="btn btn-primary no-radius modal-action-buttons-secondary mb-2"
...
<script>
    ...
    var fonts = ["Open Sans Condensed", <?php if (count($font_family) > 0) {
                                            foreach ($font_family as $single) {
                                                if ($single->value != "'Open Sans Condensed', sans-serif") {
                                                    echo '"' . $single->value . '",';
                                                }
                                            }
                                        } ?>];
