                   ...
                   <div class="col-md-1"></div>
                    ...
                    if (isset($blog->action_button_active) && $blog->action_button_active == '1') {
                        ...
                        if ($blog->btn_link == 'link') {
                            ...
                            $popupform = 'data-toggle="modal" data-is_custom_modal="YES" data-target="#modalcustomforms'.getCustomformEncodedID($blog->action_button_customform).'"';

                        }elseif ($blog->btn_link == 'eventForms') {
                            $input_link = '#';
                            $target = "";
                            $popupform = 'data-toggle="modal" data-is_custom_modal="YES" data-target="#modalcustomforms'.getCustomformEncodedID($blog->action_button_eventform).'"';
                    ...
                    ?>
                     @if($blog->btn_link == "audioiconfeature" && isset($blog->action_button_audio_icon_feature)) 
                          <span  onclick="playPauseAudio('blogAudio_<?=$blog->id ?>')" class="blog-desc-{{$blog->id}}">
                            ...
                            <a href="<?= $input_link ?>" 
                               class="btn {{$audioclass}} " style="<?php echo 'color:' . ($blog->btn_text_color ? $blog->btn_text_color . '!important;' : '#000;'); ?><?php echo 'background:' . ($blog->btn_bg ? $blog->btn_bg . ';' : '#fff;'); ?><?php echo 'font-family:' . ($blog->btn_text_font ? getfontfamily($blog->btn_text_font) . ';' : ';'); ?>"><?=$blog->btn_text?></a>
                      ...
                      <?php  $target = '';?>
                        <a
                            id="<?= $blog->id . 'blogbtn' ?>" href="<?= $input_link ?>"
                            <?php if ($blog->btn_link == "image_popup") { ?> onclick="openSlider(<?= htmlspecialchars($blog->popup_images) ?>, '<?= url('assets/uploads/' . get_current_url()) ?>');" <?php } ?>  
                        ...
                        class="btn {{$audioclass}}"  <?=$popupform?> target="<?= $target ?>" style="<?php echo 'color:' . ($blog->btn_text_color ? $blog->btn_text_color . '!important;' : '#000;'); ?><?php echo 'background:' . ($blog->btn_bg ? $blog->btn_bg . ';' : '#fff;'); ?><?php echo 'font-family:' . ($blog->btn_text_font ? getfontfamily($blog->btn_text_font) . ';' : ';'); ?>"><?=$blog->btn_text?></a>
