...
<script>
    var stripe = Stripe('pk_test_51Ppzzc02cXnldqSPaBX7z7iDN5R1WWrTP02zKY9nLG9QO6h7jNmt4Crc1RopU9y0bLcJmqgTevNVxNleI6fCu7YL00YKc0sqlV');
    // jQuery click event for elements with the 'stripe' class
    $(document).on('click', '.stripe', function() {
        var csrfToken = $('meta[name="csrf-token"]').attr('content');
        $.ajax({
            url: '/get-stripe-products',
            method: 'Get',
            headers: {
            ...
            success: function(response) {
                ...
                // Loop through the product data and append to the modal
                response.data.forEach(function(product) {
                    $('#productModal .modal-body').append(`
                <div class="product" data-product-id="${product.id}" style="cursor: pointer;">
                    <h5>${product.name}</h5>
                    <p><strong>Description:</strong> ${product.description}</p>
                    <p><strong>Price:</strong> ${product.price_amount}</p>
            ...
            createCheckoutSession(productId); // Call the function to create a checkout session
        });
    ...
    $(document).on('click', '.stripe_product', function() {
            var productId = $(this).data('package-id'); // Get the product ID
            createCheckoutSession(productId); // Call the function to create a checkout session
...
let temp_id = null;
var allDivs = $('.dropdown.back-dashboard-div');

// Check if there are at least 3 elements
if (allDivs.length >= 3) {
    // Get the third element (index 2 because it's zero-based)
    var thirdDiv = allDivs.eq(2);
    
    // Log the classes of the third element
    console.log('Classes of the third element:', thirdDiv.attr('class'));
    
    // Apply margin-top 100px if it doesn't already have it
    if (thirdDiv.css('margin-top') !== '100px') {
        thirdDiv.css('margin-top', '23px');
        thirdDiv.css('margin-right', '174px');
    }
} else {
}
function playPauseAudio(id) {
    const audioElement = document.getElementById(id);
    // If there is an audio already playing
    if (currentAudio && temp_id === id) {
        // Pause and reset if the same audio button is clicked
        currentAudio.pause();
        currentAudio.currentTime = 0;  // Reset to start
        $('.playmuteaudio').attr('src', '<?= url('assets/front/img/muted.jpg') ?>');
        currentAudio = null;
        temp_id = null;
        return; // Exit to prevent replaying the same audio
    } else if (currentAudio && temp_id !== id) {
        // If another audio is playing, stop it before playing the new one
        currentAudio.pause();
    ...
    // Play the new audio only if currentAudio is now null
    if (!currentAudio) {
    ...
    // Reset the icon and currentAudio when audio ends
